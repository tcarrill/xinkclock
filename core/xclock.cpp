...
#define BASE_RATIO          160   /* 0.15 * 1024 */
...
void xclock_draw_face(Renderer& r) 
{
    int cx = WIDTH >> 1;
    int cy = HEIGHT >> 1;
    ...
    for (int i = 0; i < 60; i++) {
        ...
        r.drawLine(inner_x, inner_y, outer_x, outer_y);
    }
}

void xclock_draw_hands(Renderer& r, ClockTime t)
{
    int cx = WIDTH >> 1;
    int cy = HEIGHT >> 1;
    int min_dim = (WIDTH < HEIGHT) ? WIDTH : HEIGHT;
    ...
    /* Hand bases */
    int base  = SCALE(radius, BASE_RATIO);
